...
describe Decidim::AttachmentCreatedEvent do
  ...
  let(:resource) { create(:attachment, attached_to: participatory_process) }
  let(:participatory_process) { create(:participatory_process, title: title) }
  let(:title) { Decidim::Faker::Localized.localized { generate(:title) } }
  let(:attached_to_url) { resource_locator(attached_to).url }
  let(:resource_title) { title["en"] }
  ...
  describe "email_subject" do
    ...
    context "when title has diacritics" do
      ...
      it "is generated correctly" do
        ...
        expect(subject.email_subject).to eq("An update to I'm a subject with diacritics !")
